...
const EmotionWidget = () => {
  const [emotionData, setEmotionData] = useState([42, 36, 15, 7]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setEmotionData(data.emotionData || [42, 36, 15, 7]); // Fallback to mock data
      } catch (error) {
  ...
  const chartData = {
    labels: ['Happy', 'Anger', 'Sadness', 'neutral'],
    datasets: [
      {
        data: emotionData,
  ...
  return (
    <div className="emotion-widget" style={{ backgroundColor: '#F8FAFE', padding: '20px', borderRadius: '10px', width: '350px', boxShadow: '0 4px 8px rgba(0,0,0,0.1)', position: 'relative' }}>
      ...
      <div className="chart-emotion-container" style={{ display: 'flex', alignItems: 'center' }}>
        ...
        <div className="emotion-details" style={{ marginLeft: '20px' }}>
          ...
          <p><span className="color-dot neutral" style={{ backgroundColor: '#5AC8FA', width: '12px', height: '12px', borderRadius: '50%', display: 'inline-block', marginRight: '8px' }}></span>neutral <span className="emotion-percentage" style={{ float: 'right' }}>{emotionData[3]}%</span></p>
        </div>
      </div>
    </div>
  );
