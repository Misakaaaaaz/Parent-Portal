...
function RightBubble({data, isLeft, onBubbleClick}) {
    ...
    const graphData = {
        datasets: [{
            data: data.map((career, index) => ({
                x: Math.random() * 30 + 20,
                y: (maxRank - career.rank + 1) * (100 / (maxRank + 1)), // Evenly space vertically
                r: (maxRank - career.rank + 1) * 15 + 30, // Size based on rank
                label: career.field,
                field: career.field
            })),
